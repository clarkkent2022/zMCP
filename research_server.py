...
def log_call(func):
    """Log function calls with their arguments and execution time."""
    if asyncio.iscoroutinefunction(func):
        @functools.wraps(func)
        async def async_wrapper(*args, **kwargs):
            func_name = func.__name__
            log_info(f"Calling: {func_name}")
            if kwargs:
                log_info(f"Arguments: {json.dumps(kwargs, indent=2, default=str)}")
            
            start_time = time.time()
            try:
                result = await func(*args, **kwargs)
                exec_time = (time.time() - start_time) * 1000
                log_success(f"{func_name} completed in {exec_time:.2f}ms")
                return result
            except Exception as e:
                log_error(f"Error in {func_name}: {str(e)}")
                raise
...
def get_local_ip() -> str:
    ...
    try:
        # Create a socket connection to an external server but don't actually connect
...
def print_server_info():
    ...
    print(f"{COLORS['GREEN']}\nAvailable tools:{COLORS['ENDC']}")
    print("  - search_papers(topic: str, max_results: int) - Search for papers on arXiv")
    print("  - extract_info(paper_id: str) - Get information about a specific paper")
    print("  - generate_search_prompt(topic: str, num_papers: int) - Generate a search prompt")
...
if __name__ == "__main__":
    try:
        ...
        sys.stderr.write("Research server stopped.\n")